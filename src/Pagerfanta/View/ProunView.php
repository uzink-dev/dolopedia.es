...
use Pagerfanta\View\Template\ProunTemplate;
 ...
 */
class ProunView extends TwitterBootstrapView
{
    ...
    private function generateContainer($pages)
    {
        ...
        $container = str_replace('%pages%', $pages, $this->template->container());
        if($this->pagerfanta->hasPreviousPage()) {
            $container = str_replace('%previous%', $this->previous(), $container);
        }
        else {
            $container = str_replace('%previous%', '', $container);
        }
        if($this->pagerfanta->hasNextPage()) {
            $container = str_replace('%next%', $this->next(), $container);
        }
        else {
            $container = str_replace('%next%', '', $container);
        }
        return $container;
    }
