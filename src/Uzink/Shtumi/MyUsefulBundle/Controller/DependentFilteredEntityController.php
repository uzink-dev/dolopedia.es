...
namespace Uzink\Shtumi\MyUsefulBundle\Controller;
...
class DependentFilteredEntityController extends Controller
{
    ...
    public function getOptionsAction()
    {
        ...
        if(stristr($entity_inf['order_property'], 'translation.'))
        {
            $properties = explode('.', $entity_inf['order_property']);
            $has_translation = $properties[0];
            $entity_inf['order_property'] = $properties[1];
        }
        ...
        {
            $qb->leftJoin('e.translations', 't')->orderBy('t.' . $entity_inf['order_property'], $entity_inf['order_direction']);
        }
        
        $qb->setParameter('parent_id', $parent_id);
